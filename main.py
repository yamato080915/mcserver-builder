...
class main(ttk.Notebook):
	def __init__(self, master=None, folder="server"):
		...
		self.proxyend = ttk.Button(self.btnframe, text="Restart", style='my.TButton', command=lambda: self.restart("proxy"))
		self.proxyend.grid(column=3, row=0)
	...
	def restart(self, server="proxy"):
		self.stop(server)
		threading.Thread(target=lambda: self.server_runner("proxy"), name="run", daemon=True).start()
	def setup(self):
