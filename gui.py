...
class main(ttk.Notebook):
    def __init__(self, master=None):
        ...
        self.buildtab.grid(row=0, column=0, sticky=tk.NSEW)
    ...
    def setup(self):
        bottom["text"] = "setting up..."
        self.main = builder.build(folder)
        bottom["text"] = "setting up...done!"
        self.display()
        if self.main.proxy:
            self.add(self.proxytab, text="Proxy")

    def display(self):
        self.lbl = tk.Label(self.buildtab, text="Build a velocity server")
        self.lbl.grid(row=0, column=0, sticky=tk.EW)
        self.buildtab.grid_columnconfigure(0, weight=1)
        self.btn = ttk.Button(self.buildtab, text="Build!", command=threading.Thread(target=self.build_proxy, name="build proxy", daemon=True).start)
        self.btn.grid(row=0, column=1)

    def build_proxy(self):
        self.main.build_proxy()
        self.main.proxy_setting()
        self.main.forward()
...
app = main(root)
app.grid(row=1, column=0, sticky=tk.NSEW)
